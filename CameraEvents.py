...
class DahuaDevice():
        ...
        try:
            if self.auth == "digest":
                image = requests.get(imageurl, stream=True,
                                     auth=requests.auth.HTTPDigestAuth(self.user, self.password)).content
            else:
                image = requests.get(imageurl, stream=True,
                                     auth=requests.auth.HTTPBasicAuth(self.user, self.password)).content
    ...
    def OnReceive(self, data):
        ...
        for Line in Data.split("\r\n"):
            ...
            if Alarm["Code"] == "IntelliFrame":
                faceData = json.loads(Alarm["data"], object_hook=lambda d: SimpleNamespace(**d))
                _LOGGER.info(
                    "Face IntelliFrame:" + Alarm["name"] + " Index: " + Alarm["channel"] + " Code: " + Alarm["Code"]
                )
                _LOGGER.info(
                    "IntelliFrame: {} ".format(faceData)
                )
            ...
            if Alarm["Code"] == "VideoMotion":
                _LOGGER.info(
                    "Video Motion received: " + Alarm["name"] + " Index: " + Alarm["channel"] + " Code: " + Alarm[
                        "Code"])
                if not self.client.connected_flag:
                    self.client.reconnect()
                ...
                if Alarm["action"] == "Start":
                    ...
                    try:
                        ...
                        regionText = "{} With {} in {} direction for {} region".format(Alarm["Code"], object, direction,
                                                                                       region)
                ...
                _LOGGER.info(
                    "dahua_event_received: " + Alarm["name"] + " Index: " + Alarm["channel"] + " Code: " + Alarm[
                        "Code"])
                if Alarm["action"] == "Start":
                    eventStart = True
                    self.client.publish(self.basetopic + "/" + Alarm["channel"] + "/event", "ON")
...
class DahuaEventThread(threading.Thread):
    ...
    def mqtt_on_picture_message(self, client, userdata, msg):
        ...
        for device in self.Devices:
            ...
            if channel > -1:
                _LOGGER.debug(
                    "Found Camera: {0} channel: {1}: Name:{2}".format(device.Name, channel, device.channels[channel]))
                if msg.payload is not None:
                    try:
                        ...
                        device.SearchImages(channel + device.snapshotoffset, d, d + datetime.timedelta(minutes=2), '',
                                            publishImages=device.publishImages, message="Snap Shot Image")
    ...
    def mqtt_on_alert_message(self, client, userdata, msg):
        ...
        for device in self.Devices:
            ...
            if device.Name == deviceName:
                ...
                self.client.publish(self.basetopic + "/" + device.Name + "/alerts/state", msg.payload, qos=0,
                                    retain=True)
